...
export async function POST(request: Request) {
  try {
    ...
    const { land_id, elapsedTime, ...formData } = body;

    // 1. Shadow Rejection (Behavioral Security)
    // If the submission is too fast (< 3s), we return 200 OK but don't save anything.
    if (elapsedTime && elapsedTime < 3000) {
      console.log(`[Shadow Reject] IP: ${ip}, Time: ${elapsedTime}ms`);
    ...
    // 4. Final Truncation (Safety Layer)
    const insertData = {
      land_id,
    ...
    // 6. Log successful/valid attempt for rate limiting
    await supabase
      .from("rate_limit_attempts")
